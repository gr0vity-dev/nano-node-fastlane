...
void nano::election::broadcast_vote_impl ()
{
	...
	if (node.config.enable_voting && node.wallets.reps ().voting > 0)
	{
		...
		if (confirmed () || have_quorum (tally_impl ()))
		{
			...
			node.nlogger.trace (nano::log::tag::election, nano::log::detail::generate_vote_final, nlogger::field ("root", root.to_string ()), nlogger::field ("hash", status.winner->hash ().to_string ()));
		...
		else
		{
			...
			node.nlogger.trace (nano::log::tag::election, nano::log::detail::generate_vote_normal, nlogger::field ("root", root.to_string ()), nlogger::field ("hash", status.winner->hash ().to_string ()));
