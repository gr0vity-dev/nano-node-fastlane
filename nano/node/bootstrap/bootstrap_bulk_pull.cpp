...
nano::bulk_pull_client::~bulk_pull_client ()
{
	...
	if (expected != pull.end && !expected.is_zero ())
	{
		...
		node->nlogger.error (nano::log::tag::bulk_pull_client, "Bulk pull end block is not expected {} for account {} or head block {}", pull.end.to_string (), pull.account_or_head.to_account (), pull.account_or_head.to_string ());
	}
	else
	{
...
void nano::bulk_pull_client::request ()
{
	...
	node->nlogger.debug (nano::log::tag::bulk_pull_client, "Requesting account {} or head block {} from {}. {} accounts in queue", pull.account_or_head.to_account (), pull.account_or_head.to_string (), connection->channel->to_string (), attempt->pulling);
	if (attempt->should_log ())
	{
		node->nlogger.info (nano::log::tag::bulk_pull_client, "Accounts in pull queue: {}", attempt->pulling);
	}
	...
	req, [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		{
			node->nlogger.error (nano::log::tag::bulk_pull_client, "Error sending bulk pull request to: {} [{}]", this_l->connection->channel->to_string (), ec.message ());
...
void nano::bulk_pull_client::received_block (boost::system::error_code ec, std::shared_ptr<nano::block> block)
{
	...
	{
		node->nlogger.error (nano::log::tag::bulk_pull_client, "Insufficient work for bulk pull block: {}", block->hash ().to_string ());
...
void nano::bulk_pull_account_client::request ()
{
	...
	node->nlogger.debug (nano::log::tag::bulk_pull_account_client, "Requesting pending for account {} from {}. Accounts in queue: {}", req.account.to_account (), connection->channel->to_string (), attempt->wallet_size ());
	if (attempt->should_log ())
	{
	...
	req, [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		{
			node->nlogger.error (nano::log::tag::bulk_pull_account_client, "Error starting bulk pull request to: {} [{}]", this_l->connection->channel->to_string (), ec.message ());
...
void nano::bulk_pull_account_client::receive_pending ()
{
	...
	connection->socket->async_read (connection->receive_buffer, size_l, [this_l, size_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		if (size_a == size_l)
		{
			...
			{
				node->nlogger.error (nano::log::tag::bulk_pull_account_client, "Error while receiving bulk pull account frontier: {}", ec.message ());
...
void nano::bulk_pull_server::set_current_end ()
{
	...
	{
		node->nlogger.debug (nano::log::tag::bulk_pull_server, "Bulk pull end block doesn't exist: {}, sending everything", request->end.to_string ());
	...
	{
		node->nlogger.debug (nano::log::tag::bulk_pull_server, "Bulk pull request for block hash: {}", request->start.to_string ());
	...
	else
	{
		...
		if (!info)
		{
			node->nlogger.debug (nano::log::tag::bulk_pull_server, "Request for unknown account: {}", request->start.to_account ());
		...
		else
		{
			...
			if (!request->end.is_zero ())
			{
				...
				{
					node->nlogger.debug (nano::log::tag::bulk_pull_server, "Request for block that is not on account chain: {} not on {}", request->end.to_string (), request->start.to_account ());
...
void nano::bulk_pull_server::send_next ()
{
	...
	if (block != nullptr)
	{
		...
		node->nlogger.trace (nano::log::tag::bulk_pull_server, nano::log::detail::sending_block, nano::nlogger::field ("block", block));
...
void nano::bulk_pull_server::sent_action (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.error (nano::log::tag::bulk_pull_server, "Unable to bulk send block: {}", ec.message ());
	}
}
...
void nano::bulk_pull_server::send_finished ()
{
	...
	node->nlogger.debug (nano::log::tag::bulk_pull_server, "Bulk sending finished");
...
void nano::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.error (nano::log::tag::bulk_pull_server, "Unable to bulk send not-a-block: {}", ec.message ());
	}
}
...
void nano::bulk_pull_account_server::set_params ()
{
	...
	{
		node->nlogger.error (nano::log::tag::bulk_pull_account_server, "Invalid bulk_pull_account flags supplied: {}", static_cast<uint8_t> (request->flags));
...
void nano::bulk_pull_account_server::send_next_block ()
{
	...
	else
	{
		 ...
		 */
		node->nlogger.debug (nano::log::tag::bulk_pull_account_server, "Done sending blocks");
...
void nano::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.error (nano::log::tag::bulk_pull_account_server, "Unable to bulk send block: {}", ec.message ());
	}
}
...
void nano::bulk_pull_account_server::send_finished ()
{
	...
	node->nlogger.debug (nano::log::tag::bulk_pull_account_server, "Bulk sending for an account finished");
...
void nano::bulk_pull_account_server::complete (boost::system::error_code const & ec, std::size_t size_a)
{
	...
	{
		node->nlogger.error (nano::log::tag::bulk_pull_account_server, "Unable to pending-as-zero: {}", ec.message ());
	}
}
